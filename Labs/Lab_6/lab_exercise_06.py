...
def count_avg_height(plants):
    ...
    height_count = 0
    for plant in plants:
        plant_height = plant[1].strip('feet')
        plant_height = int(plant_height)
        height_sum = height_sum + plant_height
        height_count = height_count + 1
    Max_Avg = round(height_sum / height_count)
    return Max_Avg
...
def filter_large_plants(plants, avg_height):
    ...
    for plant in plants:
        plant_height = plant[1].strip('feet')
        plant_height = int(plant_height)
        if plant_height > avg_height:
            big_avg.append(plant)
    return(big_avg)
