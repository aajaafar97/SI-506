...
class Droid():
    ...
    def jsonable(self):
        ...
        if self.languages:#If self.languages != None
...
class Languages():
    ...
    def add_language(self, species):
        ...
        `language_database` dictionary, this method should create a new dictionary entry with the language
        ...
        Returns:
            None
        """
        if species['language'] not in self.language_database.keys():
            self.language_database[species['language']] = [(species['name'], species['homeworld'])]
        else:
...
# Problem 4.0
def get_swapi_resource(resource, params = None, timeout = 10):
    """
